...
module.exports = async function handleMQTTMessage(topic, message, io) {
  try {
    ...
    if (topic === 'iot/sensors') {
      ...
      // Handle legacy/object flame format: {"node1": 0, "node2": 1...}
      if (sensors.flame === undefined && typeof data.flame === 'object' && data.flame !== null) {
        for (const [key, val] of Object.entries(data.flame)) {
          if (key.startsWith('node')) {
            const nodeNum = parseInt(key.replace('node', ''));
            if (!isNaN(nodeNum)) {
              sensorEntries.push({
                topic,
                floor: floorStr,
